...
{
	int* previousCoords = (int*)malloc(amountOfFocals * 2 * sizeof(int)); // tablica przechowywująca poprzednie koordynaty punktow skupienia
	...
	//przypisz 0 dla wartości poprzednich
	for (int i = 0; i < amountOfFocals; i++)
	{
	...
	//0 wartości się nie zmieniły
	//1 wartości się zmieniły
	short hasChanged = 1;
	...
	while (hasChanged)
	{
		...
		/*
			Ustawianie najkrótszej odległości dla każdego punktu w tablicy wskazywanej przez	 point ** dataPointer
		*/
		for (int i = 0; i < amountOfPoints; i++)
		{
		...
		*/
		...
		//Sprawdzenie czy koordynaty punktów skupienia się zmieniły się po między iteracjami
		for (int i = 0; i < amountOfFocals; i++)
		{
